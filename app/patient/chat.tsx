...
import { ArrowLeft, FileText, Image as ImageIcon, Paperclip, Search, Send } from 'lucide-react-native';
...
import { getConversation, getCurrentUser, getRecentChats, markMessagesAsRead, sendChatMessage } from '../../utils/api';
...
function PatientChatScreen({ onBack }: { onBack: () => void }) {
  ...
  const fetchRecentChats = async () => {
    try {
      ...
      setConversations(await getRecentChats());
  ...
  const filteredConversations = conversations.filter(c =>
    ...
    c.user.specialization?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const getInitials = (name: string) => name?.split(' ').map(n => n[0]).join('').substring(0, 2).toUpperCase() || 'DR';
  ...
  const renderChatArea = () => {
    ...
    return (
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1, backgroundColor: '#f9fafb' }}>
        {/* Header */}
        <View style={{ flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderColor: '#e5e7eb', backgroundColor: '#fff', gap: 12 }}>
          {!isDesktop && (
            <TouchableOpacity onPress={() => setSelectedChatUser(null)} style={{ padding: 8 }}>
  ...
  const renderSidebar = () => (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      <View style={{ padding: 16, borderBottomWidth: 1, borderColor: '#e5e7eb', backgroundColor: '#fff' }}>
        <TouchableOpacity onPress={onBack} style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <ArrowLeft size={20} color="#4b5563" />
          <Text style={{ color: '#4b5563', fontWeight: '500' }}>Back</Text>
        </TouchableOpacity>
        <Text style={{ fontSize: 24, fontWeight: 'bold', color: '#111827', marginBottom: 4 }}>Messages</Text>
        <Text style={{ fontSize: 13, color: '#6b7280', marginBottom: 12 }}>Chat with your dermatologists</Text>
        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: '#f3f4f6', borderRadius: 24, paddingHorizontal: 16, paddingVertical: 10, gap: 8, marginBottom: 16 }}>
          ...
          <TextInput placeholder="Search doctors..." value={searchQuery} onChangeText={setSearchQuery} style={{ flex: 1, fontSize: 14, color: '#111827' }} />
        </View>
      ...
      <ScrollView style={{ flex: 1 }}>
        {loadingChats ? (
          ...
          <View style={{ alignItems: 'center', marginTop: 60, paddingHorizontal: 32 }}>
            ...
            <Text style={{ color: '#9ca3af', fontSize: 13, marginTop: 4, textAlign: 'center' }}>Confirmed appointments will appear here.</Text>
          </View>
        ) : (
          filteredConversations.map(conv => (
            <TouchableOpacity key={conv.user._id}
              onPress={() => { setSelectedChatUser(conv.user); loadConversation(conv.user._id); }}
              style={{ flexDirection: 'row', alignItems: 'flex-start', padding: 16, borderBottomWidth: 1, borderColor: '#f3f4f6', backgroundColor: '#fff', gap: 12 }}
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ color: '#6b7280', fontSize: 12, marginBottom: 2 }}>{conv.user.specialization || 'Doctor'}</Text>
        ...
        <View style={{ flex: 1 }}>
          {selectedChatUser ? renderChatArea() : (
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#f9fafb' }}>
              <Text style={{ color: '#9ca3af', fontSize: 16 }}>Select a conversation to start chatting</Text>
            </View>
          )}
        </View>
      </View>
...
export default function ChatPage() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#fff' }}>
      <PatientChatScreen onBack={() => router.push('/')} />
    </SafeAreaView>
  );
}
